...
CancellationToken DataRepository::GetData(
  ...
  auto wait_and_check = [&] {
    if (!wait.Wait() && !context.IsCancelled()) {
      OLP_SDK_LOG_INFO_F(kLogTag, "wait_and_check() failed");
  ...
  // Get catalog first
  OLP_SDK_LOG_INFO_F(kLogTag, "GetCatalog #####");
  ...
  OLP_SDK_LOG_INFO_F(kLogTag, "getCatalog wait_and_check()");
  ...
  if (!catalog.IsSuccessful()) {
    OLP_SDK_LOG_INFO_F(kLogTag, "getCatalog '%s' unsuccessful", key.c_str());
  ...
  auto get_data = [&](const read::DataRequest& request) -> DataResponse {
    // Cache first
    OLP_SDK_LOG_INFO_F(kLogTag, "get_data - cache #####");
    ...
    OLP_SDK_LOG_INFO_F(kLogTag, "get_data - lookup #####");
    ...
    if (!api_client.IsSuccessful()) {
      OLP_SDK_LOG_INFO_F(kLogTag, "getApiClient '%s' failed", key.c_str());
      return api_client.GetError();
    }

    // 2. Get data from partition
    OLP_SDK_LOG_INFO_F(kLogTag, "get_data - blob #####");
    ...
    BlobApi::GetBlob(
        ...
        request.GetBillingTag(), boost::none, [&](DataResponse response) {
          if (response.IsSuccessful()) {
            OLP_SDK_LOG_INFO_F(kLogTag, "adding '%s' to cache", key.c_str());
            cache.Put(response.GetResult(), request.GetLayerId(),
                      request.GetDataHandle().value_or(std::string()));
          } else if (http::HttpStatusCode::FORBIDDEN ==
                     response.GetError().GetHttpStatusCode()) {
            OLP_SDK_LOG_INFO_F(kLogTag, "clearing '%s' cache", key.c_str());
            cache.Clear(request.GetLayerId(),
                        request.GetDataHandle().value_or(std::string()));
          }
  ...
  if (itr == layers.end()) {
    OLP_SDK_LOG_INFO_F(kLogTag, "Layer for '%s' doesn't exist", key.c_str());
  ...
  OLP_SDK_LOG_INFO_F(kLogTag, "GetPartitionsById #####");
  ...
  if (!partitions.IsSuccessful()) {
    OLP_SDK_LOG_INFO_F(kLogTag, "GetPartitionsById '%s' unsuccessful",
                        key.c_str());
  ...
  if (partitions_data.size() == 1u) {
    ...
    OLP_SDK_LOG_INFO_F(kLogTag, "Partiton invalid '%s'", key.c_str());
    callback(model::Data());
  }

  OLP_SDK_LOG_INFO_F(kLogTag, "done #####");
  return token;
}
